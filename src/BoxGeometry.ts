...
export default class BoxGeometry {
  ...
  createDiseGeometry() {
    ...
    this.geometry = new THREE.BoxGeometry(1, 1, 1, params.segments, params.segments, params.segments);
    ...
    for (let i = 0; i < positionAttr.count; i++) {
        ...
        const notchWave = (v: any) => {
            v = (1 / params.notchRadius) * v;
            v = Math.PI * Math.max(-1, Math.min(1, v));
            return params.notchDepth * (Math.cos(v) + 1.);
        }
        const notch = (pos: any) => notchWave(pos[0]) * notchWave(pos[1]);
...
createDiceMesh() {
    ...
    const outerMesh = new THREE.Mesh(this.createDiseGeometry(), boxMaterialOuter);
